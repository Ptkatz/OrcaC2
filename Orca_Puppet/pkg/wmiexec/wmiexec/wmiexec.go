...
func (e *wmiExecer) SetTargetBinding(binding string) error {
	if binding == "" {
		Output += fmt.Sprintln("Getting network bindings from remote host")
		targets, err := GetNetworkBindings(e.config.targetAddress)
		if err != nil {
			return err
		}
		Output += fmt.Sprintln("Resolved names, all network string bindings for host:")
		for _, name := range targets {
			Output += fmt.Sprintln("\t", name)
		}
		Output += fmt.Sprintln("Using first value as target hostname: ", targets[0])
...
func (e *wmiExecer) Auth() error {
	...
	if rsp.CommonHead.PacketType == 2 {
		Output += fmt.Sprintln("WMI Access possible!")
	}
	...
	for e.stage != "exit" {
		...
		case "Request":
			...
			case 0:
				...
				//output += fmt.Sprintlnf("hn len %d (%x)", hnLen, hnLenByte)
			...
			// output += fmt.Sprintlnf("writing stub data (call id %d): %x", callID, packetRPC.StubData[16:])
			// output += fmt.Sprintlnf("Parsed stub data: %+v", parseStub(packetRPC.StubData[16:]))
			// output += fmt.Sprintlnf("unknown portion: (%d) %x", len(parseStub(packetRPC.StubData[16:]).Unknown), parseStub(packetRPC.StubData[16:]).Unknown)
			...
			if nextStage == "Result" {
				if len(resp) > 1145 {
					Output += fmt.Sprintln("PID: ", binary.LittleEndian.Uint16(resp[1141:1145]))
					return nil
				} else {
					msg := fmt.Sprintf("Response shorter than expected... possible error in command? Expected > 1145, got ", len(resp))
...
func WMIExec(target, username, password, hash, domain, command, clientHostname, binding string, cfgIn *WmiExecConfig) (error, string) {
	runtime.GC()
	...
	if command != "" {
		if execer.targetRPCPort == 0 {
			...
			return errors.New("RPC Port is 0, cannot connect"), Output
		}
