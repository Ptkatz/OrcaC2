...
func RunCreateProcessWithPipe(shellcode []byte, prog string, args string) (stdOut, stdErr string) {
	...
	if ntStatus != 0 {
		if ntStatus == 3221225476 {
			stdErr = colorcode.OutputMessage(colorcode.SIGN_FAIL, "Error calling NtQueryInformationProcess: STATUS_INFO_LENGTH_MISMATCH")
			return
		}
	...
	if dosHeader.Magic != 23117 {
		stdErr = colorcode.OutputMessage(colorcode.SIGN_FAIL, "DOS image header magic string was not MZ")
		return
	}
	...
	if Signature != 17744 {
		stdErr = colorcode.OutputMessage(colorcode.SIGN_FAIL, "PE Signature string was not PE")
		return
	}
